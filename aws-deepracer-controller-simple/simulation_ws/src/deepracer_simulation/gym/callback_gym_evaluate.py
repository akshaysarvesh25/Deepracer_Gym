...
class DeepracerGym(gym.Env):
	...
	def reset(self):        
		...
		pose_deepracer = np.array([abs(pos[0]-self.target_point_[0]),abs(pos[1]-self.target_point_[1]), head_to_target - yaw_car],dtype=np.float32) #relative pose 
	...
	def get_reward(self,x,y):
		...
		head_to_target = self.get_heading(pose_deepracer, self.target_point_)
		pose_deepracer = np.array([abs(pos[0]-self.target_point_[0]),abs(pos[1]-self.target_point_[1]), head_to_target - yaw_car],dtype=np.float32)
		# x = pose_deepracer[0]
		# y = pose_deepracer[1]
...
actor_path = "models/sac_actor_dubins_gazebo_2"
critic_path = "models/sac_critic_dubins_gazebo_2"
